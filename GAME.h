/**La codificacion de caracteres es: ANSI.*/
	...
	void *pedir_tecla(void* argv){
        person* p1=(person*)((void**)argv)[0];
        person* p2=(person*)((void**)argv)[1];
        int* _key_codes_=(int*)((void**)argv)[2];
        short* GAME_OVER=(short*)((void**)argv)[3];
        short* cantidad_de_jugadores=(short*)((void**)argv)[4];
        char* if_pause=(char*)((void**)argv)[5];
        /*Dibujamos a los jugadores.*/
        dibujar_jugadores(p1);
	    dibujar_jugadores(p2);
	    while((*GAME_OVER!=a__QUIT__a && *GAME_OVER!=a__QUIT_Q_a && p1->vida > 0 && p2->vida > 0) || *if_pause=='Y'){
	    	*_key_codes_=getch();
 ...
 /*void inicializar(person* p,short height,int x,int y,      int vida,            chtype color,int puntaje,char* name,short modo_texto);*/
        ...
        if(pthread_create(&tid,NULL,pedir_tecla,(void*)todo)){/*Si al momento de crear el hilo hubo un error: Nesecitamos reglesar la consola en modo normal y terminar el programa.*/
          endwin();
          return __ERROR__;
        }
		...
		do{
			...
			if (GAME_OVER==a__PAUSA__a){
               ...
               crearMarcos(modo_texto);
        	   draw(jugador_1,jugador_2,nivel_de_dificulta);
        	   dibujar_jugadores(&jugador_1);
               dibujar_jugadores(&jugador_2);
           }
    	...
    	GAME_OVER=1;
		return a__QUIT__a;																						/*Si el jugador presion� la tecla salir.*/
	}
	void inicializar(person* p,short height,int x,int y,int vida,chtype color,int puntaje,char* name,short modo_texto){
		p->vida=vida;
		p->puntaje=puntaje;
	...
	int tratarkey(person* jugador_1,person* jugador_2,short cantidad_de_jugadores,int _key_codes_){
		switch (_key_codes_){/*Mediante c�digo ASSI vemos que tecla presion� el usuario.*/
			case a__ARRIBA_w__a:
            case a__ARRIBA_W__a:
				...
				dibujar_jugadores(jugador_1);/*Solo dibujamos a los jugadores si el usuario preciono la tecla correspondiente.*/ 
				break;
			case a__ABAJO_s__a:
			case a__ABAJO_S__a:
			...
			case a__ARRIBA__a:
				...
				if (jugador_2->cords.y>MIN_Y)
					jugador_2->cords.y--;
				dibujar_jugadores(jugador_2);/*Solo dibujamos a los jugadores si el usuario preciono la tecla correspondiente.*/
				break;
			case a__ABAJO__a:
				if (cantidad_de_jugadores==UN_JUGADOR)
					break;
