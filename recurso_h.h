/**La codificacion de caracteres es: ANSI.*/
	...
	*/
	...
	typedef struct{
		...
		long int puntaje;/*** La puntuaci�n del jugador.*/
		...
		coords cords;/*** The coords of person*/
            ...
            void sleep(long int milisegundos){
            ...
            }/*Si es Windows hacemos nuestra propia funcion sleep.*/
	#elif __linux__
              /*Hacemos nuestro propio comando de escape para cambiar el titulo de la stdscr en bash:*/
	          #define CAMBIAR_TITULO system("echo -e \"\\033]0;title Ping Pong de consola.\\007\" ")
              #include <unistd.h>
              /*unistd.h es por sleep.
              Si es Linux usamos una funcion ya hecha: sleep.
              ...
              */
	#endif
    void dibujar_jugadores(person* p){
         int h;
             if(p->cords.modo_texto){/*Modo solo texto.*/
                for (h=0;h < p->height;h++){
                    mvaddch(p->cords.anterior_y-h,p->cords.x,' ');
                    mvaddch(p->cords.anterior_y+h,p->cords.x,' ');
                 }
                for (h=0;h < p->height;h++){
                    mvaddch(p->cords.y-h,p->cords.x,'|');
                    mvaddch(p->cords.y+h,p->cords.x,'|');
                 }
                p->cords.anterior_y=p->cords.y;
             }else{
                 attroff(p->cords.color);/*Nos aseguramos de que alguna otra instruccion tenga el resaltado esendido y si es as?o apagamos.*/
                 for (h=0;h < p->height;h++){
                    mvaddch(p->cords.anterior_y-h,p->cords.x,' ');
                    mvaddch(p->cords.anterior_y+h,p->cords.x,' ');
                 }
                 p->cords.anterior_y=p->cords.y;/*Obtenemos las coordenadas anterior para saber donde borrar.*/
                 attron(p->cords.color);/*Prendemos el resaltado para dibujar.*/
                 for (h=0;h < p->height;h++){
                     mvaddch(p->cords.y-h,p->cords.x,'1');
                     mvaddch(p->cords.y+h,p->cords.x,'1');
                 }
                 attroff(p->cords.color);/*Apagamos el resaltado para el proximo sicro*/
             }
        }
    ...
    char inicializar_stdsrc(){
	    ...
	    if (has_colors()){
           ...
           /**En futuro debo Hacer las paletas personalizada. Recuerda: Para saber si la consola tiene soporte para una paleta personalizada usar la funcion can_change_color()*/
        }
        cbreak();                              /*Queremos que la ventana reconosca la teclas y la envie por la funcion: getch()*/
	    init_pair(1,COLOR_WHITE,COLOR_GREEN);
        keypad(stdscr,TRUE);                  /*Nesecitamos que est� en FALSE para que detenga el hilo creado y as� evitar errores.*/
    ...
    int pause(int* key_codes){
        *key_codes=0;/*No queremos que se termine apenas precionemos "p"*/
        clear();
        mvprintw(MAX_Y/2,MAX_X/3,"Precione Enter, intro, espacio o \"Q\" para seguir.");
        refresh();
        while(1)
             switch(*key_codes){
                   case a__INTRO__a:
    ...
    void mv_at_printw(int y,int x,chtype color,const char* text){
   	...
   	void inicializar(person* p,short height,int x,int y,int vida,chtype color,int puntaje,char* name,short modo_texto);	/*Aqu� inicializar� la estructura person.*/
	int tratarkey(person* jugador_1,person* jugador_2,short cantidad_de_jugadores,int _key_codes_);			/*Aqu� tratar� la tecla para mover al personaje.*/
	void tratarCoords(person* p1,person* p2,coords* pong,short indicar[ ],short boot,const int scoore,const int nivel_de_dificulta);/*Aqu� se trata la posicion de la pelota. Manipular� su coordenadas para hacer que se mueva. Tambien tratar� su interacciones con el tablero y los personajes.**/
	void dibujar_pelota(coords* pong);                 														/*Aqui dibujar� a los jugadores y la pelota en modo texto.*/
	void boot(person* p2,short indicar[ ]);																	/*Aqu� si es un solo jugador esta funcion determinar� donde esta la pelota y deacuerdo a ello se mover�*/
	void sleep(long int milisegundos);
